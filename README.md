# Hi 👋 I'm Deepak Dhanoliya

💻 Full Stack Developer | Frontend Specialist | AI-Powered Applications

I build modern web and mobile applications using React, Next.js and AI powered backend services.

---

## 🚀 Tech Stack

### Frontend
React.js • Next.js • React Native • JavaScript • TypeScript  
Redux • Tailwind CSS • HTML • CSS

### Backend
Node.js • FastAPI • Python

### Database
MongoDB • MySQL • SQL

### Tools & Technologies
Git • GitHub • REST APIs • Responsive Design • API Integration

---

## 📌 Featured Projects

🔹 DBX Studio – AI Database Management Tool  
🔹 Nissan Dashboard – Frontend React Dashboard  
🔹 Full Stack AI Web Apps  

---

## 📫 Connect With Me
LinkedIn:  https://www.linkedin.com/in/deepakdhanoliya/

Email:  ddhakad910@gmail.com
